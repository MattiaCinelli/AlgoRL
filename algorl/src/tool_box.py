...
class RLFunctions(object):
    ...
    def greedy(self, state, use_epsilon=True):
        '''Return the action with the highest state value return'''
        # All actions with highest state value return
        greedy_actions = [self.env.grid[self.env.new_state_given_action(state, action)] for action in self.env.possible_actions]
